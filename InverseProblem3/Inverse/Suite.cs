    ...
    public static class Suite
    {
        ...
        private static RBFNetworkSettings CoeffSettings =
            new RBFNetworkSettings
            {
                ...
                WeightSettings = new ParameterSettings { value = 0.0 },
                ParametersSettings =
                    new List<ParameterSettings>(new[] { new ParameterSettings { value = 0.5 } }),
        ...
        public static Case Case1(RBFNetwork.RBFNetwork direct)
        {
            ...
            ourCase.CheckPoints =
                ourCase.ControlPoints.Where(p => ((IControlPoint) p).Tag != 4 && ((IControlPoint) p).Tag != 5).Select(
                    p => (IControlPoint) p.Clone()).ToArray();
            ourCase.GraphicPoints = ourCase.CheckPoints;
